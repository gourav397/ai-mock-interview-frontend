...
function ImageEditor() {
  ...
  const [adjustments, setAdjustments] = useState({ brightness: 1, contrast: 1, saturation: 1 });
  ...
  useEffect(() => {
    return () => {
      if (sessionId) apiService.clearSession(sessionId).catch(() => {});
  ...
  const validateFile = useCallback((file) => {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      throw new Error(`Invalid file type: ${file.type || "unknown"}. Allowed: JPG, PNG, WebP`);
    }
  ...
  const applyFilter = useCallback(async (filterId) => {
    if (!sessionId) return;
    setIsProcessing(true); setErrorMessage(null);
    ...
    try {
      ...
      setErrorMessage(err.message || "Filter failed.");
      setActiveFilter(null);
    } finally {
  ...
  const applyAdjustmentsApi = useCallback(async (adj) => {
    if (!sessionId) return;
    setIsProcessing(true); setErrorMessage(null);
    ...
    try {
      ...
      const data = response.data;
      if (!data.success) throw new Error(data.error || "Adjustments failed.");
      setResultUrl(data.resultUrl ? `${API_BASE}${data.resultUrl}` : null);
      setMetadata(data.metadata);
      setImageState("done");
    } catch (err) {
  ...
  const handleAdjustmentChange = useCallback((key, value) => {
    const newVal = { ...adjustments, [key]: parseFloat(value) };
    setAdjustments(newVal);
    applyAdjustmentsApi(newVal);
  }, [adjustments, applyAdjustmentsApi]);
    ...
    try {
      ...
      switch (actionId) {
        case "enhance":
          ...
          break;
        case "upscale":
          setProcessingMessage("Upscaling 2x...");
        ...
        case "bw_q":
          ...
          break;
        case "warm_q":
          setProcessingMessage("Applying warm...");
      ...
      const data = response.data;
      if (!data.success) throw new Error(data.error || `${actionId} failed.`);
      setResultUrl(data.resultUrl ? `${API_BASE}${data.resultUrl}` : null);
      setMetadata(data.metadata);
      setImageState("done");
    } catch (err) {
  ...
  const handleAiEdit = useCallback(async (instruction) => {
    ...
    try {
      const response = await apiService.aiEditImage(sessionId, instruction.trim());
      const data = response.data;
  ...
  const handleAiEditSubmit = useCallback((e) => {
    e.preventDefault();
  ...
  const handleReset = useCallback(async () => {
    if (!sessionId) {
      setOriginalUrl(null); setResultUrl(null); setPreviewUrl(null);
      setMetadata(null); setImageState("empty"); setActiveFilter(null);
      setAdjustments({ brightness: 1, contrast: 1, saturation: 1 });
      setErrorMessage(null);
      return;
    }
    ...
    try {
      ...
      setResultUrl(null); setPreviewUrl(data.previewUrl);
      setMetadata(data.metadata); setImageState("loaded");
      setActiveFilter(null);
      setAdjustments({ brightness: 1, contrast: 1, saturation: 1 });
    } catch (err) {
  ...
  const handleNewImage = useCallback(() => {
    if (originalUrl && originalUrl.startsWith("blob:")) URL.revokeObjectURL(originalUrl);
    setOriginalUrl(null); setResultUrl(null); setPreviewUrl(null);
    setMetadata(null); setImageState("empty"); setSessionId(null);
    setActiveFilter(null);
    setAdjustments({ brightness: 1, contrast: 1, saturation: 1 });
    setErrorMessage(null); setAiInstruction("");
  ...
  const formatFileSize = (bytes) => {
    ...
    return mb < 1 ? `${(bytes / 1024).toFixed(1)} KB` : `${mb} MB`;
  };
  ...
  return (
    <div className="image-editor-container">
      <div className="image-editor-header">
        ...
        <p className="subtitle">Upload, edit, and enhance your images with AI-powered tools</p>
      </div>
      ...
      {imageState !== "empty" && (
        <div className="editor-layout">
          <div className="editor-sidebar">
            <button className="new-image-btn" onClick={handleNewImage}>📁 New Image</button>
              ...
              <div className="filter-grid">
                {FILTERS.map((filter) => (
                  <button key={filter.id}
                    ...
                    onClick={() => applyFilter(filter.id)} disabled={isProcessing}
                    title={filter.label}>
            ...
            <div className="tool-section">
              ...
              <div className="adjustment-group">
                <label>Brightness <span className="adjust-value">{adjustments.brightness.toFixed(1)}x</span></label>
              ...
              <div className="adjustment-group">
                <label>Contrast <span className="adjust-value">{adjustments.contrast.toFixed(1)}x</span></label>
              ...
              <div className="adjustment-group">
                <label>Saturation <span className="adjust-value">{adjustments.saturation.toFixed(1)}x</span></label>
              ...
              <div className="quick-actions-grid">
                {QUICK_ACTIONS.map((action) => (
                  <button key={action.id} className="quick-action-btn"
                    onClick={() => handleQuickAction(action.id)} disabled={isProcessing}>
            ...
            <div className="tool-section">
              ...
              <form onSubmit={handleAiEditSubmit} className="ai-edit-form">
                <input type="text" value={aiInstruction}
                  ...
                  disabled={isProcessing} className="ai-input" />
                <button type="submit" className="ai-edit-btn"
                  disabled={isProcessing || !aiInstruction.trim()}>Apply</button>
              </form>
            </div>
          ...
          <div className="editor-preview">
            <div className="preview-section">
              ...
              <div className="image-frame">
                {originalUrl ? (
                  <img src={originalUrl.startsWith("blob:") ? originalUrl :
                           originalUrl.startsWith("http") ? originalUrl : `${API_BASE}${originalUrl}`}
                       alt="Original" className="preview-image" />
                ) : (
              ...
              {metadata && (
                <div className="image-info">
                  <span className="info-badge">{metadata.width} × {metadata.height}</span>
                  <span className="info-badge">{metadata.format}</span>
                  {metadata.size && <span className="info-badge">{formatFileSize(metadata.size)}</span>}
                </div>
              )}
            </div>
            ...
            <div className="preview-section">
              <h3>
                ...
                {resultUrl && <a href={resultUrl} download="edited-image.jpg"
                  className="download-link">⬇ Download</a>}
              </h3>
              <div className="image-frame">
                {resultUrl ? (
                  <img src={resultUrl} alt="Edited" className="preview-image"
                       onError={(e) => { e.target.style.display = "none";
                         e.target.nextSibling.style.display = "flex"; }} />
                ) : (
                  <div className="no-image-placeholder">
                    {imageState === "loaded" ? "Apply a filter or adjustment to see the result" :
                     imageState === "processing" ? "Processing..." : "No result yet"}
                  </div>
                )}
