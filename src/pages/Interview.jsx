...
function Interview() {
  ...
  // LOAD CATEGORY — pehle login check
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (!token) {
      alert("Login required — pehle login/register karo");
      navigate("/login");
      return;
    }
    ...
    const loadCategories = async () => {
      try {
        ...
        console.log(error);
        alert("Category load nahi hui. Token expire ho gaya ho to logout karke dobara login karo.");
      }
    };
    loadCategories();
  }, []);
